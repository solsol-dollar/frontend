...
| 이름 | 값 | 사용처 |
|------|----|--------|
| `primary` / `primary-700` | `#0922AC` | CTA 버튼, 브랜드 강조, 바텀시트 헤더 |
| `primary-900` | `#061875` | 다크 강조 |
| `primary-500` | `#2545C7` | 보조 강조, 활성 상태 |
| `primary-300` | `#5B7BE5` | 링크, 서브 액션 |
| `primary-100` | `#C5D1F5` | 배경 틴트, 비활성 강조 |

```tsx
// 사용 예시
<button className="bg-primary text-white">청약신청</button>
<span className="text-primary-300">전체보기 &gt;</span>
```
...
| 이름 | 값 | 사용처 |
|------|----|--------|
| `success` | `#16A34A` | 성공 상태, 완료 메시지 |
| `danger` | `#DC2626` | 오류, 경고 액션 |
| `warning` | `#F59E0B` | 주의 안내 |
| `heart` | `#EF4444` | 찜/관심 종목 아이콘 |
...
|------|----|--------|
| `white` | `#FFFFFF` | 기본 배경 |
| `surface` | `#E8EDFB` | 카드 배경, 입력 필드 배경 |
| `surface-light` | `#F0F3FD` | 보조 배경 |
| `surface-bg` | `#F5F7FE` | 페이지 배경 |
| `surface-subtle` | `#FAFBFF` | 최소 강조 배경 |
...
| `text-secondary` | `#6B7280` | 보조 설명, 레이블 |
| `text-tertiary` | `#9CA3AF` | 비활성 상태, 힌트 |
...
// 예시
<TickerBadge ticker="CRWV" color="#29A9F5" />  // → "CR"
```
