...
export function TransferConfirmPage() {
  ...
  const fromAccountId: number = state?.fromAccountId
  const toAccountId: number = state?.toAccountId
  const sourceName: string = state?.sourceName ?? ''
  const destName: string = state?.destName ?? ''
  ...
  const sourceNum = parseFloat(sourceBalance.replace(/[^0-9.]/g, '')) || 0
  const destNum = parseFloat(destBalance.replace(/[^0-9.]/g, '')) || 0
  const sourceAfter = `$${(sourceNum - amountNum).toFixed(2)}`
  const destAfter = `$${(destNum + amountNum).toFixed(2)}`
  ...
  return (
    <div className="mobile-container flex flex-col h-screen bg-white">
      ...
      <div className="px-5 mb-10">
        ...
        {errorMsg && <p className="mt-4 text-sm text-danger text-center">{errorMsg}</p>}
      </div>
